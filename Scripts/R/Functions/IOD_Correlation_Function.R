...
IOD_vs_miRNA_plot <- function(data, Gene, parent_directory, Date) {
  ...
  gene_df <- data %>%
    ...
    select(matches("^cvi-|^Cluster_"), IOD, Genes, Sample.ID)
  ...
  for (miRNA_cluster in colnames(gene_df)) {
    ...
    # Skip Genes and IOD columns
    if (miRNA_cluster %in% c("Genes", "IOD", 'Sample.ID')) {
      next
    }
    ...
    # Extract miRNA expression and IOD
    miRNA_expression <- as.numeric(gene_df[[miRNA_cluster]])
    exp_IOD <- as.numeric(gene_df$IOD)
    
    # Create the plot
    plot <- ggplot(data = gene_df, aes(x = miRNA_expression, y = exp_IOD, label = sample_id)) +
      geom_point(aes(color = Gene), size = 2.5, alpha = 0.8) +
      ggtitle(paste(miRNA_name, "miRNA Abundance compared to IOD")) +
      geom_smooth(
        method = 'lm',
        se = TRUE,
        color = 'black',
...
IOD_vs_miRNA_plot2 <- function(data, Gene, parent_directory, Date, filter_r_squared = FALSE) {
  # Load required packages
  require(ggplot2)
  require(dplyr)
  require(ggpmisc)
  ...
  for (miRNA_cluster in colnames(gene_df)) {
    ...
    # lm_model <- lm(exp_IOD ~ miRNA_expression - 1)
    lm_model <- lm(exp_IOD ~ miRNA_expression)
...
# Example usage:
# Assuming your wider_miRNA_with_IOD_df is named wider_miRNA_with_IOD_df
...
# plots <- IOD_vs_miRNA_plot(wider_miRNA_with_IOD_df, 'Venom_ohanin', parent_directory, '2024.4.22')
