...
SVSP.reg.start = floor(8568727 / 1000 ) * 1000

SVSP.reg.end = ceiling(8981362 / 1000 ) * 1000
...
# ADD TO SVSP INFO
SVSP.info <- SVSP.info %>% left_join(VST_count_mat_filtered, by = c('gene'='Gene'))
SVSP.info <- SVSP.info %>% mutate(gene = gsub(' ','',gene))
...
SVSP.info$color = "blue"

# Plotting 
SVSP_figure <- ggplot(SVSP.info,aes(xmin = start2, xmax = end2, y = str_remove(molecule,'scaffold\\-'), fill = log10(AverageExp+1))) +
...
# Save SVSP_figure to a .png
ggsave("/Users/ballardk/Library/CloudStorage/OneDrive-UTArlington/Documents/Lab/Projects/Venom_grant/ncRNA/Scripts/R/SVMP_plot/SVSP_array_plot.pdf", SVSP_figure, width = 10, height = 2, units = "in")
