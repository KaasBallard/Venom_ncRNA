...
PLA2.reg.end = ceiling(3043778 / 1000 ) * 1000
...
# Add venom gene expression data to PLA2.info
PLA2.info <- PLA2.info %>%
  mutate(
...
# Set color for PLA2.info
PLA2.info$color = "blue"

# Plotting 
# Create plot using ggplot
PLA2_figure <- ggplot(PLA2.info,aes(xmin = start2, xmax = end2, y = str_remove(molecule,'scaffold\\-'), fill = log10(AverageExp+1))) +
