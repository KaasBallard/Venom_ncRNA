...
model_diagnostics <- function(model, directory, model_number) {
  ...
  dev.off()
  pdf_file_mu <- file.path(model_dir, "conditional_effects_mu.pdf")
  pdf(file = pdf_file_mu, width = 8.5, height = 11)
  print(conditional_effects(model, dpar = 'mu'))
  dev.off()
  pdf_file_hu <- file.path(model_dir, "conditional_effects_hu.pdf")
  pdf(file = pdf_file_hu, width = 8.5, height = 11)
  print(conditional_effects(model, dpar = 'hu'))
  dev.off()
}
...
priors <- c(
  ...
  # Fixed effects for the hu portion of the model
  ...
  # Set the prior for the in the hu portion of the model
  ...
  # Set the prior for the residual standard deviation
  prior(cauchy(0, 4), class = "sigma") # Residual standard deviation
)

# Diagnostics directory
diagnostics_path <- sprintf("Scripts/R/VariationAnalysis/2_BRMS/Model_Diagnostics/Hurdle_Lognormal/Inter_%d_2025.02.28", iterations)

## Hurdle lognormal models ----
### Venom model #11: mRNA vs miRNA numbers ----
#### Run the model ----
...
model12 <- brm(
  bf(
    ...
    hu ~ number.of.miRNAs + (1|gr(sample.id, cov = phylo_cov_matrix))
  ),
  data = miRNA_num_df,
...
t2 <- Sys.time()
timer <- t2 - t1
print(paste('Completed model12 with ', iterations, ' iterations in ', timer))
...
model12_filename <- sprintf('Data/Models/mRNA_vs_Number_of_miRNAs/Hurdle_Lognormal/Model12_with_%d_iterations_2025.02.28.rds', iterations)
# Save the model
saveRDS(model12, file = model12_filename)
rm(model12)
gc()

### Venom model #12: mRNA vs filtered miRNA numbers ----
#### Run the model ----
...
t2 <- Sys.time()
timer <- t2 - t1
print(paste('Completed model13 with ', iterations, ' iterations in ', timer))
...
#### Save the model ----
# Create a filename for the model
model13_filename <- sprintf('Data/Models/mRNA_vs_Number_of_miRNAs/Hurdle_Lognormal/Model13_with_%d_iterations_2025.02.28.rds', iterations)
# Save the model
saveRDS(model13, file = model13_filename)
rm(model13)
gc()

### Venom model #13: number of miRNAs vs Protein ----
#### Run the model ----
...
t2 <- Sys.time()
timer <- t2 - t1
print(paste('Completed model14 with ', iterations, ' iterations in ', timer))
...
#### Save the model ----
# Create a filename for the model
model14_filename <- sprintf('Data/Models/mRNA_vs_Number_of_miRNAs/Hurdle_Lognormal/Model14_with_%d_iterations_2025.02.28.rds', iterations)
# Save the model
saveRDS(model14, file = model14_filename)
rm(model14)
gc()

### Venom model #14: Protein vs filtered miRNA numbers ----
#### Run the model ----
# Get time
t1 <- Sys.time()
...
t2 <- Sys.time()
timer <- t2 - t1
print(paste('Completed model15 with ', iterations, ' iterations in ', timer))
...
#### Save the model ----
# Create a filename for the model
model15_filename <- sprintf('Data/Models/mRNA_vs_Number_of_miRNAs/Hurdle_Lognormal/Model15_with_%d_iterations_2025.02.28.rds', iterations)
# Save the model
saveRDS(model15, file = model15_filename)
rm(model15)
gc()