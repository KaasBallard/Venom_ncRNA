...
write_csv(all_samples_plot_miRNA_RNA_protein_df, file = 'Data/miRNA/miRNA_mRNA_protein_data_for_regression_analysis_2024.11.1.csv')
...
write_csv(all_samples_clr_df, file = 'Data/CLR_transformed_data/CLR_mRNA_vs_protein_expression_2024.11.1.csv')
...
#   saveRDS(venom_model8, file = paste0("Data/Models/mRNAvsProtein/venom_model8.", i, "_with_", iter, "_iterations_from_brms_2024.11.1.rds"))
# }


#### Venom modeling with miRNAs ####

# Remove unimportant columns in the miRNA data frame
miRNA_df <- all_samples_plot_miRNA_RNA_protein_df %>%
  dplyr::select(
    Genes, Venom.Family, Sample.ID, Species, miRNA.Cluster, miRNA.RPM
  )

# Get names for fusion
fusion_names <- intersect(names(miRNA_df), names(all_samples_detected_proteins_only_df))

# Create a new data frame to run the model with miRNAs
all_samples_miRNA_clr_df <- left_join(
  all_samples_detected_proteins_only_df,
  miRNA_df,
  by = fusion_names
)
...
# Set brms model parameters
warmup_percentage2 <- 0.25
chain2 <- 6
thin2 <- 500
iterations2 <- 500000
warmup2 <- iterations2 * warmup_percentage2
...
# Run the model
venom_model9 <- brm(
  CLR.Scaled.Protein.Expression ~ CLR.Scaled.mRNA.Expression * Venom.Family + (1|gr(Sample.ID, cov = phylo_cov_matrix)) + (1 + miRNA.RPM | miRNA.Cluster),
  data = all_samples_miRNA_clr_df,
  family = gaussian(link = 'identity'),
  data2 = list(phylo_cov_matrix = phylo_cov_matrix),
  control = list(max_treedepth = 12, adapt_delta = 0.9), # Increase tree depth
  chains = chain2,
...
t4 <- Sys.time()
timer2 <- t4 - t3
print(paste('Completed venom_model9 with ', iterations2, ' iterations in ', timer2))

# Save the model
saveRDS(venom_model9, file = 'Data/Models/With_miRNA/venom_model9_with_500000_iterations_from_brms_2024.11.5.rds')
