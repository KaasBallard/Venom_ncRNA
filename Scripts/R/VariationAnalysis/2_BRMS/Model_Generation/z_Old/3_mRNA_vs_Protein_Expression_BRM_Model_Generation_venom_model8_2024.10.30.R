...
write_csv(all_samples_plot_miRNA_RNA_protein_df, file = 'Data/miRNA/miRNA_mRNA_protein_data_for_regression_analysis_2024.10.30.csv')
...
write_csv(all_samples_clr_df, file = 'Data/CLR_transformed_data/CLR_mRNA_vs_protein_expression_2024.10.30.csv')
...
chain <- 6
initial_iter <- 5000000 # Add a zero
...
for (i in 1:n_runs) {
  ...
  iter <- initial_iter / (10^(i - 1)) # Halve the iterations each time
  ...
  saveRDS(venom_model8, file = paste0("Data/Models/venom_model8.", i, "_with_", iter, "_iterations_from_brms_2024.10.30.rds"))
}
