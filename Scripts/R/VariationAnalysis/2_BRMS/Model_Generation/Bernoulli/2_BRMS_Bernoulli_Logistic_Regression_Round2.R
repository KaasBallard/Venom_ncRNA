...
diagnostics_path <- sprintf("Figures/Model_Diagnostics/Bernoulli/Inter_%d_2025.04.01", iterations)


### Venom model #9: mRNA vs miRNA numbers ----
#### Run the model ----
...
model9_filename <- sprintf('Data/Models/mRNA_vs_Number_of_miRNAs/Bernoulli/Model9_with_%d_iterations_2025.04.01.rds', iterations)
# Save the model
saveRDS(model9, file = model9_filename)
rm(model9)
gc()

### Venom model #10: mRNA vs filtered miRNA numbers ----
#### Run the model ----
...
t2 <- Sys.time()
timer <- t2 - t1
print(paste('Completed model10 with ', iterations, ' iterations in ', timer))
...
#### Save the model ----
# Create a filename for the model
model10_filename <- sprintf('Data/Models/mRNA_vs_Number_of_miRNAs/Bernoulli/Model10_with_%d_iterations_2025.04.01.rds', iterations)
# Save the model
saveRDS(model10, file = model10_filename)
rm(model10)
gc()
