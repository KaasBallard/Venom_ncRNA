...
## Read in miRNA number data and the filtered miRNA number data ----
# miRNA number data
miRNA_num_df <- read_parquet(file = miRNA_number_data) %>%
  filter(
    ...
    # Remove any proteins that was not observed
    protein.observed == 'Yes',
    # Remove any truly un-observed proteins
    intensity > 0
  ) %>%
  select(
    sample.id, genes, mRNA.vst, mRNA.ntd, venom.family, number.of.miRNAs
  )
...
## Fuse the miRNA number data (both) the mRNA-Protein data ----
# Get shared columns
shared_columns <- intersect(names(mRNA_protein_df), names(miRNA_num_df))

# Join the miRNA data to the main mRNA-Protein data
miRNA_mRNA_protein_df <- full_join(
  mRNA_protein_df, miRNA_num_df,
  by = shared_columns,
) %>%
  mutate(
    number.of.miRNAs = if_else(is.na(number.of.miRNAs), 0, number.of.miRNAs)
  )
glimpse(miRNA_mRNA_protein_df)

# Join the filtered miRNA data to the main mRNA-Protein data
filt_miRNA_mRNA_protein_df <- full_join(
  mRNA_protein_df, filt_miRNA_num_df,
  by = shared_columns,
) %>%
  mutate(
    number.of.miRNAs = if_else(is.na(number.of.miRNAs), 0, number.of.miRNAs)
  )
glimpse(filt_miRNA_mRNA_protein_df)
...
### Venom model #4: protein vs mRNA with miRNA numbers ----
#### Run the model ----
# Get time
t1 <- Sys.time()
...
model4 <- brm(
  log.protein ~
    mRNA.vst * venom.family + (1 | gr(sample.id, cov = phylo_cov_matrix)) + number.of.miRNAs,
...
#### Save the model ----
# Create a filename for the model
model4_filename <- sprintf('Data/Models/mRNA_vs_Protein/Gaussian/Model4_with_%d_iterations_2025.03.13.rds', iterations)
# Save the model
...
### Venom model #5: protein vs mRNA with filtered miRNA numbers ----
#### Run the model ----
# Get time
t1 <- Sys.time()
...
model5 <- brm(
  log.protein ~
    mRNA.vst * venom.family + (1 | gr(sample.id, cov = phylo_cov_matrix)) + number.of.miRNAs,
  data = filt_miRNA_mRNA_protein_df,
  family = gaussian(link = 'identity'),
  data2 = list(phylo_cov_matrix = phylo_cov_matrix),
  control = list(max_treedepth = 12, adapt_delta = 0.95), # Increase tree depth
  seed = seed,
...
#### Save the model ----
# Create a filename for the model
model5_filename <- sprintf('Data/Models/mRNA_vs_Protein/Gaussian/Model5_with_%d_iterations_2025.03.13.rds', iterations)
# Save the model
...
#### Save the model ----
# Create a filename for the model
model6_filename <- sprintf('Data/Models/mRNA_vs_Protein/Gaussian/Model6_with_%d_iterations_2025.03.13.rds', iterations)
# Save the model
...
#### Save the model ----
# Create a filename for the model
model7_filename <- sprintf('Data/Models/mRNA_vs_Protein/Gaussian/Model7_with_%d_iterations_2025.03.13.rds', iterations)
# Save the model
