...
## This is the model for mRNA vs protein per family
# Bayesian model with gene family-specific slopes and phylogenetic random effect
...
timer <- t2 - t1
timer
model_summary <- summary(venom_model)


# Extract fitted values (posterior means) and confidence intervals
fitted_vals <- fitted(venom_model, summary = TRUE)
predicted_vals <- predict(venom_model)

# Combine original data and fitted values
all_samples_clr_df <- all_samples_clr_df %>% 
  mutate(Fitted = fitted_vals[, "Estimate"]) %>% 
  mutate(ci.upper = fitted_vals[, "Q97.5"]) %>% 
  mutate(ci.lower = fitted_vals[, "Q2.5"]) %>% 
  mutate(Predicted = predicted_vals[, "Estimate"]) %>% 
  mutate(CI.Lower = predicted_vals[, "Q2.5"]) %>% 
  mutate(CI.Upper = predicted_vals[, "Q97.5"])




# Combine residuals with original data
residuals_df <- all_samples_clr_df %>%
  mutate(Resid = residuals(venom_model)[, "Estimate"]) %>% 
  mutate(Residuals = CLR.Scaled.Protein.Expression - Fitted) %>% 
  mutate(Residuals2 = CLR.Scaled.Protein.Expression - Predicted)
...
r2_df <- residuals_df %>%
  ...
  summarise(
    R2 = 1 - sum((CLR.Scaled.Protein.Expression - Predicted)^2) / 
              sum((CLR.Scaled.Protein.Expression - mean(CLR.Scaled.Protein.Expression))^2)
  )

# Format equations and R² for plotting
r2_df$Label <- with(r2_df, 
                          paste("R² = ", round(R2, 3))
)
...
all_gene_expression_plot <- all_samples_clr_df %>%
  ggplot(aes(x = CLR.Scaled.mRNA.Expression, y = CLR.Scaled.Protein.Expression, color = Venom.Family)) +
  geom_point(size = 2.5, alpha = 0.8) +
  # Add regression line from the brm model
  geom_line(aes(y = Predicted), linewidth = 1, show.legend = F) +  # Add predicted lines
  # Add credible intervals
  geom_ribbon(aes(ymin = CI.Lower, ymax = CI.Upper, fill = Venom.Family), alpha = 0.2, show.legend = F) +  # Add credible intervals
  # Add R² values for each family
  # Use geom_text_repel to avoid overlap
  geom_text_repel(data = r2_df, aes(label = Label),
                  x = Inf, y = Inf, hjust = 1.5, vjust = 1.5,
                  direction = "y", segment.color = NA,
                  show.legend = F) +  # Avoid line segments
  # Add regression equations and R^2 values for each family
...
# Save 
# ggsave('Figures/Expression_Plots/Venom_Family_Specific/Linear_Regression_VST/All_Individuals_mRNAvsProtein_Expression_Plot_2024.10.09.pdf', plot = all_gene_expression_plot, height = 10, width = 10, create.dir = T)

# Add facet grid
all_gene_expression_plot2 <- all_gene_expression_plot + facet_wrap(~ Venom.Family)
all_gene_expression_plot2
# ggsave('Figures/Expression_Plots/Venom_Family_Specific/Linear_Regression_VST/All_Individuals_mRNAvsProtein_Expression_Plot_Individual_Families_2024.10.09.pdf', plot = all_gene_expression_plot2, height = 10, width = 10, create.dir = T)
...
## Apparently this doesn't include the effect of the phylogeny on the model
# # Get fixed effects (intercepts and slopes)
# family_coefficients <- fixef(venom_model)
# print(family_coefficients)
# 
# # Get venom families 
# venom_families <- unique(all_samples_clr_df$Venom.Family)
# venom_families
# 
# # Create a list to store the equations
# family_equations <- list()
# 
# for (venom_family in venom_families) {
#   # Extract the intercept and slope for this family
#   if (venom_family == "BPP") {
#     # Reference family, use intercept and slope as is
#     intercept <- family_coefficients["Intercept", "Estimate"]
#     slope <- family_coefficients["CLR.Scaled.mRNA.Expression", "Estimate"]
#   } else {
#     # Adjust intercept and slope for other families
#     intercept <- family_coefficients["Intercept", "Estimate"] +
#       family_coefficients[paste0("Venom.Family", venom_family), "Estimate"]
#     slope <- family_coefficients["CLR.Scaled.mRNA.Expression", "Estimate"] +
#       family_coefficients[paste0("CLR.Scaled.mRNA.Expression:Venom.Family", venom_family), "Estimate"]
#   }
#   
#   # Store the equation as a string using venom_family
#   family_equations[[venom_family]] <- paste0("y = ", round(intercept, 3), " + ", round(slope, 3), " * x")
# }
# 
# # Print the equations
# family_equations
...
# 
# 
# # Create a data frame for the regression lines for each venom family
# line_data <- data.frame()
# 
# # Calculate the necessary values based on the equations
# for (venom_family in venom_families) {
#   # Extract the intercept and slope from family_equations
#   intercept <- as.numeric(gsub("y = (.+) \\+ (.+) \\* x", "\\1", family_equations[[venom_family]]))
#   slope <- as.numeric(gsub("y = (.+) \\+ (.+) \\* x", "\\2", family_equations[[venom_family]]))
#   
#   # Generate a sequence of CLR.Scaled.mRNA.Expression values for predictions
#   mrna_vals <- seq(min(all_samples_clr_df$CLR.Scaled.mRNA.Expression), 
#                 max(all_samples_clr_df$CLR.Scaled.mRNA.Expression), 
#                 length.out = 200)
#   
#   # Calculate corresponding y values using the regression equation
#   predicted_protein_vals <- intercept + slope * mrna_vals
#   
#   # Create a temporary data frame
#   temp_df <- data.frame(
#     CLR.Scaled.mRNA.Expression = mrna_vals,
#     CLR.Scaled.Protein.Expression = predicted_protein_vals,
#     Venom.Family = venom_family
#   )
#   
#   # Append to the line data
...
# 
# # Create figure for mRNA and Protein expression
# all_gene_expression_plot3 <- all_samples_clr_df %>%
#   ggplot(aes(x = CLR.Scaled.mRNA.Expression, y = CLR.Scaled.Protein.Expression, color = Venom.Family)) +
#   geom_point(size = 2.5, alpha = 0.8) +
#   geom_line(data = line_data, aes(color = Venom.Family), linewidth = 1, linetype = 'dashed', show.legend = F) +  # Add regression lines
#   scale_x_continuous() +  # Set x-axis to continuous scale
#   scale_y_continuous() +  # Set y-axis to continuous scale
#   labs(
#     title = 'a. mRNA vs Protein',
...
#     legend.title.position = 'top'
#   ) 
# all_gene_expression_plot3
# 
# # Add facet grid for different venom families
# all_gene_expression_plot4 <- all_gene_expression_plot3 + facet_wrap(~ Venom.Family)
# # Print the plot
# all_gene_expression_plot4
...
#### Other Ways of doing the brm ####
# # Generate conditional effects from the model (includes regression lines)
# conditional_effects_data <- conditional_effects(venom_model, effects = "CLR.Scaled.mRNA.Expression:Venom.Family")
# 
# # Extract the plot from conditional_effects (this contains the model's predicted lines)
# gg_conditional_effects <- plot(conditional_effects_data, plot = FALSE)[[1]]  # Get the first plot (for mRNA expression)
# 
# 
# 
# 
# 
# # Create the plot, adding points for observed data and regression lines from model predictions
# all_gene_expression_plot3 <- ggplot(
#   all_samples_clr_df,
#   aes(x = CLR.Scaled.mRNA.Expression, y = CLR.Scaled.Protein.Expression, color = Venom.Family)
# ) +
#   geom_point(size = 2.5, alpha = 0.8) +  # Observed data points
#   geom_line(
#     data = gg_conditional_effects$data,  # Add regression lines from the model
#     aes(x = effect1__, y = estimate__, color = Venom.Family),
#     linewidth = 0.25,
#     show.legend = F
#   ) +
#   # geom_ribbon(
#   #   data = gg_conditional_effects$data,  # Add confidence intervals
#   #   aes(x = effect1__, ymin = lower__, ymax = upper__, fill = Venom.Family),
...
#     color = 'Venom Family'
#   ) +
#   scale_color_manual(values = venom_colors) +  # Apply color scheme for points/lines
#   # scale_fill_manual(values = venom_colors) +  # Apply color scheme for ribbons
#   theme_classic() +
#   theme(
#     plot.title = element_text(hjust = 0.5, face = 'bold', margin = margin(b = 5, t = 5), size = 15),
...
# Save 
# ggsave('Figures/Expression_Plots/miRNA_vs_Protein/Linear_Regression_VST/All_Individuals_microRNAvsProtein_Expression_Plot_2024.10.09.pdf', plot = miRNA_vs_protein_expression_plot, create.dir = T)
...
# #  plots <- miRNA_vs_protein_plot2(wider_miRNA_VST_protein_df, gene, path, '2024.10.09', filter_r_squared = F)
# # }
# 
# 
...
# # for (gene in venom_genes) {
# #  # Run the function in a loop
# #  plots <- miRNA_vs_protein_plot2(wider_miRNA_VST_protein_df, gene, path2, '2024.10.09', filter_r_squared = T)
# # }
# 
# 
# 
...
#     miRNA_vs_protein_plot2(wider_miRNA_VST_protein_df, gene, path, '2024.10.09', filter_r_squared = FALSE)
#     gc()
#   }
# }, mc.cores = num_cores)
# 
...
#   for (gene in gene_batch) {
#     # Run your plot function for each gene
#     miRNA_vs_protein_plot2(wider_miRNA_VST_protein_df, gene, path2, '2024.10.09', filter_r_squared = TRUE)
#     gc()
#   }
# }, mc.cores = num_cores)
...
# ggsave("Figures/Expression_Plots/miRNA_vs_Protein/Bubble_Plot/Binding_Data_vs_RSquared/miRNA_Binding_Energy_vs_RSquared_Regressions_2024.10.09.pdf", plot = be_vs_r_squared_plot, width = 10, height = 10, dpi = 900, create.dir = T)
...
# ggsave("Figures/Expression_Plots/miRNA_vs_Protein/Bubble_Plot/Binding_Data_vs_RSquared/Labeled/Labeled_miRNA_Binding_Energy_vs_RSquared_Regressions_2024.10.09.pdf", plot = be_vs_r_squared_plot2, width = 25, height = 25, dpi = 900, limitsize = F, create.dir = T)
...
# ggsave("Figures/Expression_Plots/miRNA_vs_Protein/Bubble_Plot/Binding_Data_vs_RSquared/miRNA_Binding_Score_vs_RSquared_Regressions_2024.10.09.pdf", plot = bs_vs_r_squared_plot, width = 10, height = 10, dpi = 900, create.dir = T)
...
# ggsave("Figures/Expression_Plots/miRNA_vs_Protein/Bubble_Plot/Binding_Data_vs_RSquared/Labeled/Labeled_miRNA_Binding_Score_vs_RSquared_Regressions_2024.10.09.pdf", plot = bs_vs_r_squared_plot2, width = 25, height = 25, dpi = 900, create.dir = T)
...
# ggsave("Figures/Expression_Plots/miRNA_vs_Protein/Bubble_Plot/RSquared_and_Binding_Score_2024.10.09.pdf", plot = r_squared_binding_score_bubble_plot, width = 15, height = 28, dpi = 900, create.dir = T)
...
# ggsave("Figures/Expression_Plots/miRNA_vs_Protein/Bubble_Plot/RSquared_and_Binding_Energy_2024.10.09.pdf", plot = r_squared_binding_energy_bubble_plot, width = 15, height = 28, dpi = 900, create.dir = T)
...
# ggsave("Figures/Expression_Plots/miRNA_vs_Protein/Bubble_Plot/Filtered_RSquared_and_Binding_Energy_2024.10.09.pdf", plot = filtered_r_squared_binding_energy_bubble_plot, width = 10, height = 15, dpi = 900, create.dir = T)
...
# ggsave("Figures/Expression_Plots/miRNA_vs_Protein/Bubble_Plot/Filtered_RSquared_and_Binding_Score_2024.10.09.pdf", plot = filtered_r_squared_binding_score_bubble_plot, width = 10, height = 15, dpi = 900, create.dir = T)
...
R_squared_heatmap
# ggsave("Figures/Expression_Plots/miRNA_vs_Protein/Heatmaps/Filtered_RSquared_Heatmap_(BS=160_BE=-20)_2024.10.09.pdf", plot = R_squared_heatmap, width = 10, height = 15, dpi = 900, create.dir = T)
...
# write.csv(max_scores_df2, 'Tables/Expression_Tables/miRNA_vs_Protein/Top30_Best_miRNA-Gene_Pairs_2024.10.09.csv', row.names = F)
...
# ggsave("Figures/Expression_Plots/miRNA_vs_Protein/Heatmaps/Filtered_RSquared_Heatmap_Best_30_2024.10.09.pdf", plot = R_squared_heatmap_best_30, width = 10, height = 15, dpi = 900, create.dir = T)
...
# ggsave("Figures/Expression_Plots/miRNA_vs_Protein/Bubble_Plot/Top30_RSquared_and_Binding_Score_2024.10.09.pdf", plot = r_squared_binding_score_bubble_plot_top_30, width = 10, height = 15, dpi = 900, create.dir = T)
...
# ggsave("Figures/Expression_Plots/miRNA_vs_Protein/Bubble_Plot/Top30_RSquared_and_Binding_Energy_2024.10.09.pdf", plot = r_squared_binding_energy_bubble_plot_top_30, width = 10, height = 15, dpi = 900, create.dir = T)
...
# Save
# ggsave('Figures/Expression_Plots/miRNA_vs_Protein/Linear_Regression_VST/Viridis_Only_Individuals_mRNAvsProtein_Expression_Plot_2024.10.09.pdf', plot = viridis_only_plot, create.dir = T)
...
# Save
# ggsave('Figures/Expression_Plots/miRNA_vs_Protein/Linear_Regression_VST/CV1081_viridis_mRNAvsProtein_Expression_Plot_2024.10.09.pdf', plot = CV1081_plot, create.dir = T)
...
# Save
# ggsave('Figures/Expression_Plots/miRNA_vs_Protein/Linear_Regression_VST/CV0857_viridis_mRNAvsProtein_Expression_Plot_2024.10.09.pdf', plot = CV0857_plot, create.dir = T)
...
# Save
# ggsave('Figures/Expression_Plots/miRNA_vs_Protein/Linear_Regression_VST/CV1086_viridis_mRNAvsProtein_Expression_Plot_2024.10.09.pdf', plot = CV1086_plot, create.dir = T)
...
# Save
# ggsave('Figures/Expression_Plots/miRNA_vs_Protein/Linear_Regression_VST/CV1087_viridis_mRNAvsProtein_Expression_Plot_2024.10.09.pdf', plot = CV1087_plot, create.dir = T)
...
# Save
# ggsave('Figures/Expression_Plots/miRNA_vs_Protein/Linear_Regression_VST/CV0987_lutosus_mRNAvsProtein_Expression_Plot_2024.10.09.pdf', plot = CV0987_plot, create.dir = T)
...
# Save
# ggsave('Figures/Expression_Plots/miRNA_vs_Protein/Linear_Regression_VST/CV0985_concolor_mRNAvsProtein_Expression_Plot_2024.10.09.pdf', plot = CV0985_plot, create.dir = T)
...
# Didn't work... at all
# Zero correlation
# ggsave('Figures/Expression_Plots/miRNA_vs_Protein/Linear_Regression_VST/All_Individuals_mRNAvsmicroRNA_Expression_Plot_2024.10.09.pdf', plot = miRNA_vs_mRNA_all_samples_plot, create.dir = T)
...
figure_6
# ggsave("Figures/1_Main_Figures/Figure_6/Figure_6_2024.10.09.pdf", plot = figure_6, width = 18, height = 30, dpi = 2400, create.dir = T)
