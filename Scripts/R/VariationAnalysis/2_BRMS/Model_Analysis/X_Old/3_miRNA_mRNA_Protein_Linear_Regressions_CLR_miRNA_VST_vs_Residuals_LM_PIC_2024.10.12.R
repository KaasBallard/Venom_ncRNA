...
tree_file <- 'Data/Tree/11Snake_Tree_ViridisPolytomy_edited_2024.10.17.phy'
...
missing_samples <- c(
  # "'CV1084_viridis_Mid_M'", "'CV1095_viridis_North_M'",
  # "'CV1089_viridis_South_M'", "'CV1090_cerberus_Other_M'",
  "'CV1082_viridis_South_M'"
)
...
# Root the tree
# Set the two oreganus samples as the outgroup
outgroup <-  c("RVG.7S.CV0985.concolor.Other.F", "RVG.6S.CV0987.lutosus.Other.M")

# Set outgroup
snake_tree_rooted <- root(snake_tree, outgroup = outgroup, resolve.root = T)

# Recompute branch lengths after pruning
snake_tree_rooted <- ape::compute.brlen(snake_tree_rooted)

# Check the snake tree to see what it looks like
# Make tree plot
tree_plot2 <- ggtree(snake_tree_rooted, layout = 'rectangular', size = 1) +
  geom_tiplab(size = 4, angle = 0) +
  geom_text2(
    aes(label = round(branch.length, 2), subset = !isTip),
    vjust = -0.5,
...
# It looks like the pic() function only takes one observations per sample in the vector you give it so I will have to split the data frame
# Split the data frame by gene
split_gene_df <- split(all_samples_clr_df, all_samples_clr_df$Genes)

# I will have to fix this when I root the tree
# Function that should split the 
apply_pic_to_gene <- function(gene_df, tree) {
  # Ensure the data is ordered by the tree's tip labels
  gene_df <- gene_df %>%
    arrange(factor(Sample.ID, levels = tree$tip.label))

  # Apply the phylogenetic contrast
  gene_df <- gene_df %>%
    mutate(mRNA.PIC = ape::pic(CLR.Scaled.mRNA.Expression, tree)) %>%
    mutate(Protein.PIC = ape::pic(CLR.Scaled.Protein.Expression, tree))

  return(gene_df)
}


# Apply pic to each gene and combine them back into one data frame
all_samples_pic_df <- lapply(split_gene_df, apply_pic_to_gene, tree = snake_tree_rooted) %>%
  bind_rows()

# View the final data frame with PIC values
print(all_samples_pic_df)


# Fit linear models for each Venom Family
models <- all_samples_clr_df %>%
  group_by(Venom.Family) %>%
  do(model = lm(CLR.Scaled.Protein.Expression ~ CLR.Scaled.mRNA.Expression, data = .))


smvp9_df <- all_samples_clr_df %>% 
  filter(str_detect(Genes, 'SVMP9')) %>% 
  arrange(factor(Sample.ID, levels = snake_tree_rooted$tip.label)) # %>% 
  # mutate(mRNA.PIC = ape::pic(CLR.Scaled.mRNA.Expression, snake_tree_rooted)) %>% 
  # mutate(Protein.PIC = ape::pic(CLR.Scaled.Protein.Expression, snake_tree_rooted))

mRNA_pic <- ape::pic(smvp9_df$CLR.Scaled.mRNA.Expression, snake_tree_rooted)
protein_pic <- ape::pic(smvp9_df$CLR.Scaled.Protein.Expression, snake_tree_rooted)


print(length(mRNA_pic))
print(length(protein_pic))
...
residuals_df <- all_samples_clr_df %>%
  ...
  do({
    ...
    data.frame(Sample.ID = .$Sample.ID, Genes = .$Genes, Residuals = residuals(model), Fitted = fitted(model))
  })
...
all_samples_residuals_miRNA_df <- left_join(
  ...
  mutate(
    Res = CLR.Scaled.Protein.Expression - Fitted
  ) %>% 
  left_join(
