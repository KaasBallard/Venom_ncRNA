...
summary_stats <- tidy(venom_model2)
# Get individual observations data
augment <- augment(venom_model2)
# Model stats
glance <- glance(venom_model2)

# Get predicted values
predictions <- predict(venom_model2, newdata = all_samples_clr_df, re.form = NA, se.fit = T)
...
t_val <- qt(1 - alpha / 2, df = df.residual(venom_model2))  # t-value for CI
ci_lower <- fitted_values - t_val * se_fit
ci_upper <- fitted_values + t_val * se_fit

# Create shared names set
shared_names <- intersect(names(augment), names(all_samples_clr_df))
...
all_samples_clr_df2 <- all_samples_clr_df %>% 
  ...
  mutate(
    Residuals = CLR.Scaled.Protein.Expression - .fitted
  )
# Get rows where they differ
differences_df <- all_samples_clr_df2 %>% filter(.fitted != Fitted)



# Create a new data frame to hold an expanded set of predicted values for smoothing purposes
prediced_vals_df <- all_samples_clr_df %>%
  # Create a grid of values for CLR.Scaled.mRNA.Expression for each Venom.Family and Genes combination
  group_by(Venom.Family, Genes) %>%
  summarize(
    mRNA.Seq = list(
      seq(min(CLR.Scaled.mRNA.Expression), max(CLR.Scaled.mRNA.Expression), length.out = 100)
    )
  ) %>%
  unnest(cols = c(mRNA.Seq)) %>%
  # Create a new data frame for predictions
  mutate(
    Predicted = predict(
      venom_model2,
      newdata = data.frame(
        CLR.Scaled.mRNA.Expression = mRNA.Seq,
        Venom.Family = Venom.Family,
        Genes = Genes
      ),
      re.form = NULL
    )
  ) %>%
  rename(CLR.Scaled.mRNA.Expression = mRNA.Seq)
