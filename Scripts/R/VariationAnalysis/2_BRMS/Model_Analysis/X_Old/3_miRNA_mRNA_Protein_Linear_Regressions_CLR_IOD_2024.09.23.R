...
# Create a second data frame that only has observed proteins
mi_df2 <- mi_df %>% 
  filter(
...
all_samples_clr_df <- all_samples_plot_df %>%
  # Filter out proteins that were zero
  dplyr::filter(
    Intensity > 0
  ) %>%
  group_by(Sample.ID) %>%
...
# Save 
ggsave('Figures/Expression_Plots/Index_Of_Discordance/Linear_Regression_VST/All_Individuals_mRNAvsProtein_Expression_Plot_2024.09.23.pdf', plot = all_gene_expression_plot, create.dir = T)




#### Data Frame For Index of Discordance ####

# Create a data frame for all of the venom genes where Intensity and mRNA expression are all in the same column
venom_mRNA_protein_df <- mi_df %>% # Use the mi_df so that I can still do something about the zero expression data
  filter(
    str_starts(Genes, 'Venom_'),
  ) %>%
  dplyr::select(
...
# Pivot data frame so that all samples protein expression levels are in a single column
all_samples_protein_df <- venom_mRNA_protein_df %>% 
  dplyr::select(-contains('miRNA.'), -contains('Intensity')) %>%
  distinct() %>%
  pivot_longer(
    cols = contains('RNA.VST.'),
...
# Pivot data frame so that all samples mRNA expression levels are in a single column
all_samples_mRNA_df <- venom_mRNA_protein_df %>% 
  dplyr::select(-contains('miRNA.'), -contains('RNA.')) %>% 
  distinct() %>% 
  pivot_longer(
    cols = contains('Intensity.'),
...
# Add pseudo count for protein expression level
pseudo_count_protein <- 1e-6

# Fuse them back together and calculate IOD (Index of discordance)
mRNA_protein_IOD_df <- left_join(
  all_samples_protein_df,
  all_samples_mRNA_df,
  by = c('Sample.ID', 'Genes', 'Venom.Family')
) %>% 
  mutate(Pseudo.Intensity = ifelse(Intensity == 0, pseudo_count_protein, Intensity)) %>% 
  #  Calculate an index of discordance to see if it can be a better measure than the residuals
  mutate(
    IOD = log(Pseudo.Intensity + 1) / RNA.VST
  )
...
# Create a set of shared columns between the above and the data frame with IOD
columns_shared <- intersect(names(miRNA_mRNA_protein_pivot_df), names(mRNA_protein_IOD_df))
...
wider_miRNA_with_IOD_df <- miRNA_with_IOD_df %>% 
  pivot_wider(
...
#### IOD vs miRNAs correlate ####

# Create figure for miRNA vs IOD
all_IOD_vs_miRNA_expression_plot <- ggplot(miRNA_with_IOD_df, aes(x = miRNA.RPM, y = IOD)) +
  # ggplot(aes(x = all_mRNA_expression, y = all_protein_expression, label = gene_label)) +
  geom_point(aes(color = Venom.Family), size = 2.5, alpha = 0.8) +
  geom_smooth(
    method = 'lm',
  ...
  scale_y_continuous() +  # Set y-axis to continuous scale
  labs(
    title = 'miRNA expression vs Index of Discordance',
    x = 'miRNA expression (rpm)',
    y = 'Index of Discordance'
  ) +
  scale_color_manual(values = venom_colors) +  # Apply color scheme  
  theme_classic() +
  theme(
...
all_IOD_vs_miRNA_expression_plot
# No relationship, dammit
...
source('Scripts/R/Protein-miRNA_RNA_Analysis/Functions/IOD_Correlation_Function.R')

# Create a list of venom genes for a function to iterate through
venom_genes <- wider_miRNA_with_IOD_df %>% distinct(Genes)
venom_genes <- venom_genes$Genes
# Create path for the plots to go in
path <- 'Figures/Expression_Plots/Index_Of_Discordance/Linear_Regression_VST/x_Individual_miRNA_Gene_Relationships'

# # Loop the function
# for (gene in venom_genes) {
#  # Run the function in a loop
#  plots <- residuals_vs_miRNA_plot2(wider_miRNA_with_residuals_df, gene, path, '2024.09.22', filter_r_squared = F)
# }


# Add only the plots with R squared higher than
# Create a second path
path2 <- 'Figures/Expression_Plots/Index_Of_Discordance/Linear_Regression_VST/x_Individual_miRNA_Gene_Relationships/Good_R_squared'

# # Loop through the function
# for (gene in venom_genes) {
#  # Run the function in a loop
#  plots <- residuals_vs_miRNA_plot2(wider_miRNA_with_residuals_df, gene, path2, '2024.09.22', filter_r_squared = T)
# }
...
batch_size <- 1  # Adjust this depending on your system's memory limits
...
mclapply(gene_batches, function(gene_batch) {
  for (gene in gene_batch) {
    ...
    IOD_vs_miRNA_plot2(wider_miRNA_with_IOD_df, gene, path, '2024.09.23', filter_r_squared = FALSE)
    gc()
  }
}, mc.cores = num_cores)
...
mclapply(gene_batches, function(gene_batch) {
  for (gene in gene_batch) {
    # Run your plot function for each gene
    IOD_vs_miRNA_plot2(wider_miRNA_with_IOD_df, gene, path2, '2024.09.23', filter_r_squared = TRUE)
    gc()
  }
}, mc.cores = num_cores)
...
shared_column_names_for_origin_fusion = intersect(names(origin_df), names(miRNA_with_IOD_df))


# Create a data frame that contains residauls and other important data
miRNA_with_IOD_and_origin_df <- left_join(
  miRNA_with_IOD_df,
  origin_df,
  ...
  dplyr::select(
    Sample.ID, Genes, Venom.Family, IOD, miRNA.Cluster, Total.Score, Total.Energy, miRNA.RPM, Origin
  ) %>% 
  mutate(
...
calc_r_squared_and_slope <- function(data) {
  model <- lm(IOD ~ miRNA.RPM, data = data)
...
r_squared_df <- miRNA_with_IOD_and_origin_df %>% 
...
ggsave("Figures/Expression_Plots/Index_Of_Discordance/Bubble_Plot/Binding_Data_vs_RSquared/miRNA_Binding_Energy_vs_RSquared_Regressions_2024.09.23.pdf", plot = be_vs_r_squared_plot, width = 10, height = 10, dpi = 900, create.dir = T)
...
ggsave("Figures/Expression_Plots/Index_Of_Discordance/Bubble_Plot/Binding_Data_vs_RSquared/Labeled/Labeled_miRNA_Binding_Energy_vs_RSquared_Regressions_2024.09.23.pdf", plot = be_vs_r_squared_plot2, width = 25, height = 25, dpi = 900, limitsize = F, create.dir = T)
...
ggsave("Figures/Expression_Plots/Index_Of_Discordance/Bubble_Plot/Binding_Data_vs_RSquared/miRNA_Binding_Score_vs_RSquared_Regressions_2024.09.23.pdf", plot = bs_vs_r_squared_plot, width = 10, height = 10, dpi = 900, create.dir = T)
...
ggsave("Figures/Expression_Plots/Index_Of_Discordance/Bubble_Plot/Binding_Data_vs_RSquared/Labeled/Labeled_miRNA_Binding_Score_vs_RSquared_Regressions_2024.09.23.pdf", plot = bs_vs_r_squared_plot2, width = 25, height = 25, dpi = 900, create.dir = T)
...
ggsave("Figures/Expression_Plots/Index_Of_Discordance/Bubble_Plot/RSquared_and_Binding_Score_2024.09.23.pdf", plot = r_squared_binding_score_bubble_plot, width = 15, height = 28, dpi = 900, create.dir = T)
...
ggsave("Figures/Expression_Plots/Index_Of_Discordance/Bubble_Plot/RSquared_and_Binding_Energy_2024.09.23.pdf", plot = r_squared_binding_energy_bubble_plot, width = 15, height = 28, dpi = 900, create.dir = T)
...
ggsave("Figures/Expression_Plots/Index_Of_Discordance/Bubble_Plot/Filtered_RSquared_and_Binding_Energy_2024.09.23.pdf", plot = filtered_r_squared_binding_energy_bubble_plot, width = 10, height = 15, dpi = 900, create.dir = T)
...
ggsave("Figures/Expression_Plots/Index_Of_Discordance/Bubble_Plot/Filtered_RSquared_and_Binding_Score_2024.09.23.pdf", plot = filtered_r_squared_binding_score_bubble_plot, width = 10, height = 15, dpi = 900, create.dir = T)
...
R_squared_heatmap
ggsave("Figures/Expression_Plots/Index_Of_Discordance/Heatmaps/Filtered_RSquared_Heatmap_(BS=160_BE=-20)_2024.09.23.pdf", plot = R_squared_heatmap, width = 10, height = 15, dpi = 900, create.dir = T)
...
write.csv(max_scores_df2, 'Tables/Expression_Tables/Index_Of_Discordance/Top30_Best_miRNA-Gene_Pairs_2024.09.22.csv', row.names = F)
...
ggsave("Figures/Expression_Plots/Index_Of_Discordance/Heatmaps/Filtered_RSquared_Heatmap_Best_30_2024.09.23.pdf", plot = R_squared_heatmap_best_30, width = 10, height = 15, dpi = 900, create.dir = T)
...
ggsave("Figures/Expression_Plots/Index_Of_Discordance/Bubble_Plot/Top30_RSquared_and_Binding_Score_2024.09.23.pdf", plot = r_squared_binding_score_bubble_plot_top_30, width = 10, height = 15, dpi = 900, create.dir = T)
...
ggsave("Figures/Expression_Plots/Index_Of_Discordance/Bubble_Plot/Top30_RSquared_and_Binding_Energy_2024.09.23.pdf", plot = r_squared_binding_energy_bubble_plot_top_30, width = 10, height = 15, dpi = 900, create.dir = T)
...
# Save 
ggsave('Figures/Expression_Plots/Index_Of_Discordance/Linear_Regression_VST/Viridis_Only_Individuals_mRNAvsProtein_Expression_Plot_2024.09.23.pdf', plot = viridis_only_plot, create.dir = T)
...
# Save 
ggsave('Figures/Expression_Plots/Index_Of_Discordance/Linear_Regression_VST/CV1081_viridis_mRNAvsProtein_Expression_Plot_2024.09.23.pdf', plot = CV1081_plot, create.dir = T)
...
# Save 
ggsave('Figures/Expression_Plots/Index_Of_Discordance/Linear_Regression_VST/CV0857_viridis_mRNAvsProtein_Expression_Plot_2024.09.23.pdf', plot = CV0857_plot, create.dir = T)
...
# Save 
ggsave('Figures/Expression_Plots/Index_Of_Discordance/Linear_Regression_VST/CV1086_viridis_mRNAvsProtein_Expression_Plot_2024.09.23.pdf', plot = CV1086_plot, create.dir = T)
...
# Save 
ggsave('Figures/Expression_Plots/Index_Of_Discordance/Linear_Regression_VST/CV1087_viridis_mRNAvsProtein_Expression_Plot_2024.09.23.pdf', plot = CV1087_plot, create.dir = T)
...
# Save 
ggsave('Figures/Expression_Plots/Index_Of_Discordance/Linear_Regression_VST/CV0987_lutosus_mRNAvsProtein_Expression_Plot_2024.09.23.pdf', plot = CV0987_plot, create.dir = T)
...
# Save 
ggsave('Figures/Expression_Plots/Index_Of_Discordance/Linear_Regression_VST/CV0985_concolor_mRNAvsProtein_Expression_Plot_2024.09.23.pdf', plot = CV0985_plot, create.dir = T)
